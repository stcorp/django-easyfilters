...
from .queries import date_aggregation, numeric_range_counts, value_counts
...
class Filter(object):
    ...
    def __init__(self,
        ...
        if self.field_obj.remote_field is not None:
            self.rel_model = self.field_obj.remote_field.model
            self.rel_field = self.field_obj.remote_field.get_related_field()
...
class ManyToManyFilter(ChooseAgainMixin, RelatedObjectMixin, Filter):
    ...
    def get_values_counts(self, qs):
        ...
        fkey_this = [f for f in through._meta.fields
                     if f.remote_field is not None and f.remote_field.model is self.model][0]
        fkey_other = [f for f in through._meta.fields
                      if f.remote_field is not None and f.remote_field.model is rel_model][0]
...
class DateTimeFilter(RangeFilterMixin, Filter):
    ...
    def get_choices_add(self, qs):
        ...
        def get_choices_add_recursive(chosen):
            ...
            results = date_aggregation(qs, self.field, range_type.label)
